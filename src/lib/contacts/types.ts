...
/** The kinds of address the API accepts. Anything else is a 422. */
export const ADDRESS_TYPES = ["Home", "Work", "Other"] as const;

export type AddressType = (typeof ADDRESS_TYPES)[number];

/** `AddressRead` — one address, always nested under the contact that owns it. */
...
/** An address as written: same shape without the server-assigned id. */
...
export interface Contact {
  ...
  notes: string | null;
  /** Base64 data URL, ready to drop into an `<img src>`. */
  photo: string | null;
  addresses: Address[];
  created_at: string;
  updated_at: string;
...
/** The contact's own scalar fields — everything a `Field` can render. */
